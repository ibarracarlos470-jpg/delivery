...
export default function OrderChat({ orderId, myRole, myName }: Props) {
  ...
  const color = myRole === 'DRIVER' ? 'bg-orange-500' : 'bg-green-600'

  return (
    <div className="fixed bottom-5 right-4 sm:right-6 z-50">
      {/* Chat window — full screen on mobile, fixed on desktop */}
      {open && (
        <div className={`
          mb-3 bg-white shadow-2xl border flex flex-col overflow-hidden
          fixed sm:relative inset-x-2 sm:inset-auto bottom-20 sm:bottom-auto
          top-4 sm:top-auto rounded-2xl sm:w-80
        `} style={{ maxHeight: 'calc(100dvh - 100px)' }}>
          {/* Header */}
          <div className={`flex items-center justify-between px-4 py-3 ${color} text-white shrink-0`}>
            <div>
              <p className="font-bold text-sm">Chat con {otherLabel}</p>
              <p className="text-xs opacity-75">Pedido #{orderId.slice(-6).toUpperCase()}</p>
            </div>
            <button onClick={() => setOpen(false)} className="hover:opacity-75 transition-opacity p-1">
              <X size={20} />
            </button>
          </div>
          ...
          {/* Input */}
          <form onSubmit={send} className="flex gap-2 p-3 border-t bg-white shrink-0">
            <input
              value={text}
              onChange={e => setText(e.target.value)}
              placeholder="Escribe un mensaje..."
              className="flex-1 text-sm border rounded-xl px-3 py-2.5 outline-none focus:border-green-400 min-w-0"
              maxLength={500}
            />
            <button type="submit" disabled={sending || !text.trim()}
              className={`p-2.5 rounded-xl text-white transition-colors disabled:opacity-40 shrink-0 ${
      ...
      <button onClick={() => setOpen(o => !o)}
        className={`relative w-14 h-14 rounded-full text-white shadow-lg flex items-center justify-center transition-transform hover:scale-105 active:scale-95 ${color}`}>
